...
public class PDFPreviewPopup {
    ...
    /**
     * Show a PDF preview from a File. Reads the file into bytes and delegates.
     */
    public static void show(File pdfFile, String title, String defaultFileName, StackPane rootStack) {
        try {
            byte[] bytes = Files.readAllBytes(pdfFile.toPath());
            show(bytes, title, defaultFileName, rootStack);
        } catch (IOException e) {
            LOG.severe("Cannot read PDF file: " + e.getMessage());
        }
    }

    /**
     * Show a PDF preview using a Scene's root. Wraps the root in a StackPane if needed.
     */
    public static void showInScene(byte[] pdfBytes, String title, String defaultFileName, javafx.scene.Scene scene) {
        if (scene == null) return;
        Parent root = scene.getRoot();
        if (root instanceof StackPane sp) {
            show(pdfBytes, title, defaultFileName, sp);
        } else {
            // Wrap the existing root in a StackPane so we can overlay
            StackPane wrapper = new StackPane();
            wrapper.getChildren().add(root);
            scene.setRoot(wrapper);
            show(pdfBytes, title, defaultFileName, wrapper);
        }
    }

    /**
     * Show a PDF preview from a File using a Scene's root.
     */
    public static void showInScene(File pdfFile, String title, String defaultFileName, javafx.scene.Scene scene) {
        try {
            byte[] bytes = Files.readAllBytes(pdfFile.toPath());
            showInScene(bytes, title, defaultFileName, scene);
        } catch (IOException e) {
            LOG.severe("Cannot read PDF file: " + e.getMessage());
        }
    }
