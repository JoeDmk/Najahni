...
public class ClientDashboardController {
    ...
    public void initialize() {
        ...
        // ── Auto-load projects from session ──
        models.User currentUser = SessionService.getInstance().getCurrentUser();
        if (currentUser != null && connectedUserId == 0) {
    ...
    private void ouvrirDetailsProjet(Projet projet) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/views/projets/DetailsProjet.fxml"));
            Scene scene = new Scene(loader.load());
            scene.getStylesheets().add(getClass().getResource("/views/projets/projets.css").toExternalForm());
    ...
    private void retourAccueil() {
        tools.NavigationHelper.goHome(projetsTable);
    }
