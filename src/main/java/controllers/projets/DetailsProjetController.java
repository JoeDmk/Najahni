...
import services.projets.IAScoringService;
import services.projets.NewsService;
import services.projets.PdfRapportService;
import services.projets.WorldBankService;
        ...
        new Thread(() -> {
            try {
                ...
                Platform.runLater(() -> {
                    ...
                    try {
                        java.io.File pdfFile = new java.io.File(cheminPdf);
                        byte[] pdfBytes = java.nio.file.Files.readAllBytes(pdfFile.toPath());
                        javafx.scene.Scene scene = btnTelechargerPdf.getScene();
                        if (scene != null) {
                            util.PDFPreviewPopup.showInScene(pdfBytes,
                                    "Rapport — " + projet.getTitre(),
                                    "rapport_" + sanitiser(projet.getTitre()) + ".pdf",
                                    scene);
                        }
                    } catch (Exception ex) {
