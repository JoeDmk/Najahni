...
import services.community.AiEventService;
import services.community.EmailAsync;
import services.community.EventParticipantCRUD;
import services.community.NotificationEmailService;
...
public class EventDetailsController {
    ...
    private void validateByUploadingQrImage() {
        ...
        try {
            String payload = services.community.QrDecodeUtil.decodeFromImage(file);
            boolean ok = validatePayload(payload);
    ...
    private boolean validatePayload(String payload) {
        ...
        String expected = services.community.TicketSigner.signature(
    ...
    private void leaveEvent() {
        ...
        if (isCurrentUserCreator()) {
            ...
            try {
                ...
                // Delete entire event
                new services.community.EventCRUD().supprimer(event.getId());
    ...
    private void joinEvent() {
        try {
            ...
            services.community.EmailAsync.run(() -> {
                new services.community.NotificationEmailService()
    ...
    private void goBack() {
        try {
            Parent root = FXMLLoader.load(
                    getClass().getResource("/views/community/EventsPage.fxml")
            );
    ...
    private void handleGoHome() {
        tools.NavigationHelper.goHome(participantsContainer);
    }
}
