...
import services.investissement.RiskCalculator;
import services.investissement.RiskResult;
import services.investissement.RiskService;
...
public class InvestmentOpportunityController {
    ...
    private void setupTableColumns() {
        ...
        colTargetAmount.setCellValueFactory(cellData ->
            new SimpleStringProperty(cellData.getValue().getFormattedAmount(getUserCurrency(), currencyService)));
    ...
    private void loadSummary() {
        ...
        double convertedTotal = currencyService.convert(totalAmount.doubleValue(), "EUR", getUserCurrency());
        lblTotalAmount.setText(CurrencyService.format(convertedTotal, getUserCurrency()));
        updateCurrencyDisplay();
    }
    ...
    private void closeOpportunity(InvestmentOpportunity opp) {
        if (AlertUtils.showConfirmation("Fermer l'Opportunité",
                "Fermer cette opportunité de " + opp.getFormattedAmount(getUserCurrency(), currencyService) + " ?\nAucune nouvelle offre ne pourra être soumise.")) {
    ...
    private void calculateRisk(InvestmentOpportunity opp) {
        ...
        String confirmMsg = "Calculer le Risk Score IA pour cette opportunité ?\n\n"
            + "📊 Montant : " + opp.getFormattedAmount(getUserCurrency(), currencyService) + "\n"
    ...
    public void saveOpportunity() {
        try {
            ...
            if (targetAmount.compareTo(new BigDecimal("10000000")) > 0) {
                throw new IllegalArgumentException("Le montant cible ne peut pas dépasser " + CurrencyService.format(10_000_000, getUserCurrency()) + ".");
            }
