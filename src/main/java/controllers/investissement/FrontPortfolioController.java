...
public class FrontPortfolioController {
    ...
    private void updateStats(List<InvestmentOffer> offers) {
        if (offers.isEmpty()) {
            lblTotalInvested.setText(CurrencyService.format(0, getUserCurrency()));
            lblTotalCount.setText("0");
            lblAvgAmount.setText(CurrencyService.format(0, getUserCurrency()));
            return;
        }
        ...
        lblTotalInvested.setText(CurrencyService.format(
                currencyService.convert(total.doubleValue(), "EUR", getUserCurrency()), getUserCurrency()));
        lblTotalCount.setText(String.valueOf(offers.size()));
        lblAvgAmount.setText(CurrencyService.format(
                currencyService.convert(avg.doubleValue(), "EUR", getUserCurrency()), getUserCurrency()));
    }
    ...
    private VBox createPortfolioCard(InvestmentOffer offer) {
        ...
        Label amountLabel = new Label(offer.getFormattedAmount(getUserCurrency(), currencyService));
    ...
    private void showDetailPopup(InvestmentOffer offer) {
        ...
        body.getChildren().add(buildSection("💼", "DÉTAILS DE L'OFFRE",
            buildDetailRow("💰 Montant investi", offer.getFormattedAmount(getUserCurrency(), currencyService)),
        ...
        VBox paymentSection = buildSection("🔐", "DÉTAILS DU PAIEMENT",
            ...
            buildDetailRow("💱 Devise", getUserCurrency()),
