...
import services.investissement.*;
...
public class FrontOffersController {
    ...
    private VBox createOfferCard(InvestmentOffer offer) {
        ...
        Label lblAmount = new Label(offer.getFormattedAmount(getUserCurrency(), currencyService));
    ...
    private void showDetailPopup(InvestmentOffer offer) {
        ...
        body.getChildren().add(buildSection(
            "💼", "DÉTAILS DE L'OFFRE",
            buildDetailRow("💰 Montant proposé", offer.getFormattedAmount(getUserCurrency(), currencyService)),
        ...
        VBox oppSection = buildSection(
            ...
            buildDetailRow("🎯 Montant cible", opportunity.getFormattedAmount(getUserCurrency(), currencyService)),
    ...
    private VBox buildPopupHeader(InvestmentOffer offer) {
        ...
        Label subtitle = new Label(offer.getFormattedAmount(getUserCurrency(), currencyService));
    ...
    private void buildStripePaymentFooter(VBox footer, InvestmentOffer offer,
        ...
        Label lblSummary = new Label("Montant : " + offer.getFormattedAmount(getUserCurrency(), currencyService));
    ...
    private void showCardEntryForm(InvestmentOffer offer, InvestmentOpportunity opportunity,
        ...
        // ── Amount summary ──
        Label amountLabel = new Label("💰 Montant à payer : " + offer.getFormattedAmount(getUserCurrency(), currencyService));
            ...
            paymentService.createPaymentIntent(amountCents, getUserCurrency().toLowerCase(), description)
                .thenAccept(result -> Platform.runLater(() -> {
                    ...
                    if (result.isSuccess()) {
                        ...
                        try {
                            models.User currentUser = services.SessionService.getInstance().getCurrentUser();
                            if (currentUser != null && currentUser.getEmail() != null && !currentUser.getEmail().isBlank()) {
                                String fullName = (currentUser.getFirstname() + " " + currentUser.getLastname()).trim();
                                String projectName = project != null ? project.getTitle() : "Projet #" + opportunity.getId();
                                new services.investissement.PaymentEmailService()
                                    .sendPaymentConfirmation(
                                        currentUser.getEmail(),
                                        fullName,
                                        offer.getFormattedAmount(getUserCurrency(), currencyService),
                                        projectName,
                                        result.getPaymentIntentId(),
                                        offer.getId()
                                    );
                                System.out.println("[EMAIL] Payment confirmation email queued for " + currentUser.getEmail());
                            }
                        } catch (Exception emailEx) {
                            System.err.println("[EMAIL] Could not send confirmation: " + emailEx.getMessage());
                        }
    ...
    public void submitOffer() {
        ...
        try {
            ...
            int investorId = SessionService.getInstance().getCurrentUser().getId();
