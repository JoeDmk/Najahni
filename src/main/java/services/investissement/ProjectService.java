...
public class ProjectService {
    ...
    public List<Project> findByEntrepreneur(int entrepreneurId) {
        List<Project> projects = new ArrayList<>();
        // Check both entrepreneur_id and user_id since the projets module stores creator as user_id
        String sql = "SELECT * FROM projet WHERE entrepreneur_id = ? OR user_id = ? ORDER BY date_creation DESC";
        try (PreparedStatement ps = cnx.prepareStatement(sql)) {
            ps.setInt(1, entrepreneurId);
            ps.setInt(2, entrepreneurId);
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) projects.add(mapResultSetToProject(rs));
            }
        } catch (SQLException e) {
            // Fallback: if columns don't exist, return all
    ...
    private Project mapResultSetToProject(ResultSet rs) throws SQLException {
        ...
        // Fallback: if entrepreneur_id is 0, use user_id (projets module stores creator as user_id)
        if (project.getEntrepreneurId() == 0) {
            try { project.setEntrepreneurId(rs.getInt("user_id")); }
