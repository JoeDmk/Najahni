...
public class InvestmentPDFService {
    ...
    /** Retourne la devise pr\u00e9f\u00e9r\u00e9e de l'utilisateur connect\u00e9 (ou EUR par d\u00e9faut). */
    ...
    /** Formate un montant dans la devise de l'utilisateur. */
    private String fmtCurrency(double amount) {
        String c = getUserCurrency();
        double converted = currencyService.convert(amount, "EUR", c);
        return CurrencyService.format(converted, c);
    }

    private String fmtCurrency(BigDecimal amount) {
        return fmtCurrency(amount.doubleValue());
    }
    ...
    public byte[] generatePaymentReceipt(InvestmentOffer offer, InvestmentOpportunity opportunity, Project project) {
        try (ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
            ...
            Paragraph amountPara = new Paragraph(offer.getFormattedAmount(getUserCurrency(), currencyService), BIG_AMOUNT_FONT);
            amountPara.setAlignment(Element.ALIGN_CENTER);
            doc.add(amountPara);
            Paragraph eurLabel = new Paragraph("Montant pay\u00e9 (" + getUserCurrency() + ")", LABEL_FONT);
            ...
            addDetailRow(detailTable, "Montant propos\u00e9", offer.getFormattedAmount(getUserCurrency(), currencyService));
            if (opportunity != null) {
                addDetailRow(detailTable, "Opportunit\u00e9", opportunity.getDescription() != null ? opportunity.getDescription() : "N/A");
                addDetailRow(detailTable, "Montant cible", opportunity.getFormattedAmount(getUserCurrency(), currencyService));
            ...
            addDetailRow(paymentTable, "Devise", getUserCurrency());
    ...
    public byte[] generatePortfolioReport(List<InvestmentOffer> paidOffers,
        ...
        try (ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
            ...
            addSummaryCard(summaryTable, "Total Investi", fmtCurrency(total), PRIMARY);
            addSummaryCard(summaryTable, "Nb. Investissements", String.valueOf(paidOffers.size()), ACCENT);
            addSummaryCard(summaryTable, "Montant Moyen", fmtCurrency(avg), PRIMARY_DARK);
            ...
            for (InvestmentOffer offer : paidOffers) {
                ...
                addTableCell(investTable, offer.getFormattedAmount(getUserCurrency(), currencyService), rowBg);
    ...
    public byte[] generateOpportunitySheet(InvestmentOpportunity opportunity, Project project) {
        try (ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
            ...
            // ── Grand montant ──
            Paragraph amountPara = new Paragraph(opportunity.getFormattedAmount(getUserCurrency(), currencyService), BIG_AMOUNT_FONT);
            amountPara.setAlignment(Element.ALIGN_CENTER);
            doc.add(amountPara);
            Paragraph targetLabel = new Paragraph("Montant cible (" + getUserCurrency() + ")", LABEL_FONT);
